...
bool CRegionCaptureThread::Start()
{
	...
	if (!GF()->CheckWindow(m_region_capture_draw_region.GetSafeHwnd()))
	{
...
{
	if (!GF()->CheckWindow(GetSafeHwnd()) ||
		!GF()->CheckWindow(m_region_capture_draw_region.GetSafeHwnd()))
		return false;
...
void CRegionCaptureThread::CWork::Pause()
{
	if (!m_region_capture_thread || 
		!GF()->CheckWindow(m_region_capture_thread->GetSafeHwnd()))
		return;
