...
bool CWindowCaptureThread::Start()
{
	...
	if (!GF()->CheckWindow(m_window_capture_draw_region.GetSafeHwnd()))
	{
...
{
	if (!GF()->CheckWindow(GetSafeHwnd()) ||
		!GF()->CheckWindow(m_window_capture_draw_region.GetSafeHwnd()))
		return false;
...
void CWindowCaptureThread::CWork::Pause()
{
	if (!m_window_capture_thread || 
		!GF()->CheckWindow(m_window_capture_thread->GetSafeHwnd()))
		return;
