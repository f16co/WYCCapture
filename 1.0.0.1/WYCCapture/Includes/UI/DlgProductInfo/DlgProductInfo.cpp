...
bool CDlgProductInfo::Create()
{
	...
	product_info_label_string.Format(_T("%s (%s)"), GI()->GetProductName(), GI()->GetVersion());
...
void CDlgProductInfo::Show()
{
	if (!GF()->CheckWindow(GetSafeHwnd()) ||
		!GF()->CheckWindow(m_ok_button.GetSafeHwnd()) ||
		!GF()->CheckWindow(m_product_info_label.GetSafeHwnd()))
		return;
