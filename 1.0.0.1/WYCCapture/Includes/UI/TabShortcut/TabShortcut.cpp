...
{
	if (!GF()->CheckWindow(m_shortcut_list_ctrl.GetSafeHwnd()))
		return;
...
bool CTabShortcut::InitListCtrl()
{
	...
	m_shortcut_list_ctrl.InsertColumn(2, _T("Name"), LVCFMT_CENTER, shortcut_list_ctrl_rect.Width() * 1 / 4);
	m_shortcut_list_ctrl.InsertColumn(3, _T("Shortcut"), LVCFMT_CENTER, shortcut_list_ctrl_rect.Width() * 2 / 4);
...
{
	if (!GF()->CheckWindow(m_input_shortcut_edit.GetSafeHwnd()))
		return;
...
void CTabShortcut::OnPaint()
{
	...
	CRect shortcut_list_ctrl_rect = GF()->GetScreenRect(GetSafeHwnd(), m_shortcut_list_ctrl.GetSafeHwnd());
	CRect input_shortcut_edit_rect = GF()->GetScreenRect(GetSafeHwnd(), m_input_shortcut_edit.GetSafeHwnd());
...
{
	if (!GF()->CheckWindow(m_input_shortcut_edit.GetSafeHwnd()) ||
		!GF()->CheckWindow(m_shortcut_list_ctrl.GetSafeHwnd()))
		return;
