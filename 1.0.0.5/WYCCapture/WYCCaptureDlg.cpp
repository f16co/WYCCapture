...
BEGIN_MESSAGE_MAP(CWYCCaptureDlg, CDialogEx)
	...
	ON_COMMAND(ID_TRAY_ICON_CAPTURE, OnTrayIconCapture)
	ON_COMMAND(ID_TRAY_ICON_CHANGE_CAPTURE_MODE, OnTrayIconChangeCaptureMode)
...
void CWYCCaptureDlg::MakePopupMenu()
{
	...
	for (int i = 0; i < (int)shorcut_command_name_list.size(); i++)
	{
		if (shorcut_command_name_list[i].Find(_T("Capture")) >= 0)
			::AppendMenu(m_context_menu, MF_STRING, ID_TRAY_ICON_CAPTURE, CMyString(_T("%s\t%s"), shorcut_command_name_list[i], CShortcutManager::GetSingletone()->MakeShortcutStrinig(shortcut_code_list[i])));
		else if (shorcut_command_name_list[i].Find(_T("Change Capture Mode")) >= 0)
			::AppendMenu(m_context_menu, MF_STRING, ID_TRAY_ICON_CHANGE_CAPTURE_MODE, CMyString(_T("%s\t%s"), shorcut_command_name_list[i], CShortcutManager::GetSingletone()->MakeShortcutStrinig(shortcut_code_list[i])));
