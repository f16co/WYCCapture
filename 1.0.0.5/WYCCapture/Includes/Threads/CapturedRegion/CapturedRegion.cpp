...
bool CCapturedRegion::Create()
{
	...
	::GetClassName(GetSafeHwnd(), class_name, MAX_PATH);
	
	WNDCLASSEX wnd_class = { 0, };
	GetClassInfoEx(AfxGetInstanceHandle(), class_name, &wnd_class);
...
void CCapturedRegion::OnPaint()
{
	...
	DrawCustomLine(m_overlay_image.GetDC(), client_rect);
	m_overlay_image.ReleaseDC();

	DrawCustomErase(m_overlay_image.GetDC(), client_rect);
	m_overlay_image.ReleaseDC();
...
void CCapturedRegion::DrawCustomLine(HDC mem_dc, CRect& client_rect)
{
	...
	{
		::SetPixel(mem_dc, (int)roundf(next_point.X), (int)roundf(next_point.Y), m_custom_draw_pen_color);
	}

	memG.DrawLine(&pen, prev_point, next_point);
}

void CCapturedRegion::DrawCustomErase(HDC mem_dc, CRect& client_rect)
{
...
void CCapturedRegion::DestroyPopupMenu()
{
	if (m_context_menu)
	{
...
BOOL CCapturedRegion::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}
