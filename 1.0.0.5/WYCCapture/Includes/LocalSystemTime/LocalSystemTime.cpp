...
CLocalSystemTime::CLocalSystemTime(WORD year, WORD month, WORD day, WORD hour, WORD min, WORD sec, WORD ms)
{
	ZeroMemory(&m_time, sizeof(SYSTEMTIME));
...
bool CLocalSystemTime::IsValid()
{
	CLocalSystemTime local_time(true);

	if (IsNullTime())
		return false;

	if (m_time.wYear < 1901 || m_time.wYear > local_time.GetYear())
		return false;

	if (m_time.wMonth == 0 || m_time.wMonth > 12)
		return false;

	if (m_time.wDay == 0)
		return false;

	if (!(0 <= m_time.wHour && m_time.wHour < 24))
		return false;

	if (!(0 <= m_time.wMinute && m_time.wMinute < 60))
		return false;

	if (!(0 <= m_time.wSecond && m_time.wSecond < 60))
		return false;

	if (!(0 <= m_time.wMilliseconds && m_time.wMilliseconds < 1000))
		return false;

	return true;
}

bool CLocalSystemTime::IsValid(SYSTEMTIME& time)
{
	CLocalSystemTime target_time(time);
	return target_time.IsValid();
}
...
CString CLocalSystemTime::ToString(bool millisecond_view /*= false*/, bool date /*= true*/, bool time /*= true*/, DATE_FORMAT date_format /*= DF_UNKNOWN*/)
{
	CString time_string;
	if (date)
...
CString CLocalSystemTime::SysTimeToStr(SYSTEMTIME& sys_time, bool millisecond_view /*= false*/, bool date /*= true*/, bool time /*= true*/, DATE_FORMAT date_format /*= DF_UNKNOWN*/)
{
	CLocalSystemTime local_time(sys_time);
	return local_time.ToString(millisecond_view, date, time, date_format);
}
...
void CLocalSystemTime::FromLocal(SYSTEMTIME& local_time)
{
	m_time = CLocalSystemTime(local_time).ToUTC();
}

SYSTEMTIME CLocalSystemTime::LocalToUTC(SYSTEMTIME& local_time)
{
	return CLocalSystemTime(local_time).ToUTC();
}

SYSTEMTIME CLocalSystemTime::ToLocal()
{
	FILETIME utc_file_time, local_file_time;
	SYSTEMTIME local_time;
	ZeroMemory(&local_time, sizeof(SYSTEMTIME));
	SystemTimeToFileTime(&m_time, &utc_file_time);
...
{
	m_time = CLocalSystemTime(utc_time).ToLocal();
}

SYSTEMTIME CLocalSystemTime::UTCToLocal(SYSTEMTIME& utc_time)
{
	return CLocalSystemTime(utc_time).ToLocal();
}
...
{
	CLocalSystemTime local_time_l(l);
	CLocalSystemTime local_time_r(r);

	return (local_time_l == local_time_r);
}
